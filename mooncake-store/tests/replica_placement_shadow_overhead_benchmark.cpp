...
enum class Profile { QUIESCENT, ACTIVE_INTENTS };

const char* ProfileName(Profile profile) {
    return profile == Profile::QUIESCENT ? "quiescent" : "active_intents";
}

ReplicaPlacementShadowConfig EvaluatorConfig(Profile profile) {
    ...
    config.policy.targets[static_cast<size_t>(ReplicaTemperature::WARM)] =
        profile == Profile::QUIESCENT
            ? std::array{ReplicaTierTarget{1, true},
                         ReplicaTierTarget{0, false},
                         ReplicaTierTarget{0, false},
                         ReplicaTierTarget{0, false}}
            : std::array{
...
Context MakeContext(Profile profile, Mode mode) {
    MasterServiceConfig config;
    if (mode != Mode::OFF) {
        MasterReplicaPlacementShadowConfig shadow;
        shadow.evaluator = EvaluatorConfig(profile);
    ...
    segment.name =
        std::string("overhead_") + ProfileName(profile) + '_' + ModeName(mode);
    ...
    const std::string key = std::string("benchmark_key_") +
                            ProfileName(profile) + '_' + ModeName(mode);
    ...
    return Context{profile, mode, std::move(service), client_id, key, {}};
}
...
int main() {
    ...
    std::array<Context, 6> contexts = {
        MakeContext(Profile::QUIESCENT, Mode::OFF),
        MakeContext(Profile::QUIESCENT, Mode::EXTERNAL_SNAPSHOT),
        MakeContext(Profile::QUIESCENT, Mode::AUTO_COLLECTOR),
        MakeContext(Profile::ACTIVE_INTENTS, Mode::OFF),
        MakeContext(Profile::ACTIVE_INTENTS, Mode::EXTERNAL_SNAPSHOT),
        MakeContext(Profile::ACTIVE_INTENTS, Mode::AUTO_COLLECTOR)};
    ...
    uint64_t successes = 0;
    std::cout << "profile,mode,sample,iterations,total_ns,ns_per_get\n";
    for (size_t sample = 0; sample < kSamples; ++sample) {
        for (size_t offset = 0; offset < contexts.size(); ++offset) {
            ...
            std::cout << ProfileName(context.profile) << ','
                      << ModeName(context.mode) << ',' << sample << ','
                      << kIterations << ',' << elapsed << ',' << std::fixed
                      << std::setprecision(3) << ns_per_get << '\n';
        }
    }

    std::cout << "summary,profile,mode,p50_ns_per_get,p95_ns_per_get,"
                 "overhead_vs_off_p50_percent\n";
    for (const auto& context : contexts) {
        const size_t baseline_index =
            context.profile == Profile::QUIESCENT ? 0 : 3;
        const double baseline_p50 =
            Percentile(contexts[baseline_index].samples, 0.50);
        ...
        std::cout << "summary," << ProfileName(context.profile) << ','
                  << ModeName(context.mode) << ',' << p50 << ',' << p95 << ','
                  << overhead << '\n';
    }
