...
// LEGACY is the only mode that consults the process-global V1 scorer. SHADOW
// and ACTIVE use the scorer-free structural policy as their baseline so V2
// instances cannot affect one another through global state.
...
struct ReplicaSelectionDecision {
    ...
    // Populated for SHADOW/ACTIVE. LEGACY leaves this empty to avoid scanning
    // the descriptor list once for diagnostics and again for the V1 policy.
...
// Immutable after construction. No global V1 scorer is read in SHADOW or
// ACTIVE mode. Diagnostics are opt-in so the common selection path does not
